...
- How are MESSAGE and MACRO linked? I presume via energy system costs, i.e. EC, which is then again included in the capital constraint. But how exactly does that link work?
  A qualitative description of the MESSAGE-MACRO, including initial conditions of MACRO, the calibration procedure, and the iteration process used to be part of the MACRO documentation ([see earlier version](https://github.com/iiasa/message_doc/blob/v2017/source/annex/macro.rst)), but it seems was deleted unintentionally in the most recent release.
- Also, How (if at all) are capital and capacity linked? Which linkages does total capital stock have otherwise (only net capital formation is indirectly included in production function)?
  As you rightly state, production and capital formation are affected by the MESSAGE results. The MESSAGE variable `CAP` and the related investments contribute to the energy system cost variable `EC` via the `total_cost` and also the `eneprice` parameters in MACRO. Note that in developed economies the energy sector typically contributes a few percent to total investments and capital, while in developing countries and energy exporters the share can be larger.
- Why is COST_NODAL_NET calculated in two different files: MESSAGE_MACRO_run.gms and model_solve.gms?
  There is actually a bug related to the calculation of `COST_NODAL_NET` and the energy system costs `total_cost` transferred to MACRO in the process of being fixed [PR#430](https://github.com/iiasa/message_ix/pull/430). The duplication of the calculation is not necessary and there is unit difference between the two.
 Same for GDP/Y in MESSAGE_MACRO_run.gms and macro_core.gms (I notice that the nodes are declared differently; what's the reason behind that?)
 I don't see `GDP` being calculated in `macro_core.gms`. Nodes are defined differently in MACRO and MESSAGE, as not all regions represented in MESSAGE need to be economy related. For example, there is 'GLB' region in MESSAGE where interregional trade and international shipping is represented. This region is not represented in MACRO.
- For TOTAL_PRODUCTION and other variables in MACRO, it says that energy sectors are not included - is that true given that NEWENE is included via NEW_PRODUCTION and via PRODENE/PHYSENE/EC?
  You are right, the statement in the documentation that energy sectors are not included in production is not correct and needs to be adjusted. As mentioned before, the MACRO documentation needs an overhaul. So its good that you point these issues out.
- What are the roles of demand_fixed, DEMAND.L, and DEMAND.I?
  The parameter `demand_fixed` is not related to MARCO, but to MESSAGE and is assigned to the parameter `demand` (see also [Documentation](https://docs.messageix.org/en/stable/model/MESSAGE/parameter_def.html#parameters-of-the-demand-section)). The postfix `.L` refers to the level of a variable in GAMS. `DEMAND.L` is set to the demand `demand_fixed` used in the previous MESSAGE run in `model_solve.gms`. This has just been introduced to be symmetric with `PRICE_COMMODITY.L` where some post-processing calculation is necessary (undiscounting of prices). There is no `.I` suffix in GAMS, so I assume that this a lower case `L` as GAMS is insensitive to lower/upper case spelling.
